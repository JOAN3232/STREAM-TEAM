...
import { getBackdropUrl, getBrowseContent, getPosterUrl } from "../services/tmdbService";
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#090909] text-white" onClick={() => { if (noticeOpen) setNoticeOpen(false); if (profileOpen) setProfileOpen(false); }}>
    <header className={`fixed inset-x-0 top-0 z-50 transition-colors duration-500 ${scrolled ? "bg-[#090909] shadow-lg" : "bg-gradient-to-b from-black/90 via-black/50 to-transparent"}`}>
      <div className="flex h-16 items-center px-4 md:px-12">
        ...
        <nav className="hidden items-center gap-5 lg:flex">{[["Home",null],["TV Shows","tv"],["Movies","movies"],["New & Popular","popular"],["My List",null]].map(([label,key]) => <button key={label} type="button" onClick={() => key ? goToRow(key) : label === "Home" ? window.scrollTo({top:0,behavior:"smooth"}) : null} className={`text-[13px] transition hover:text-white/65 ${label === "Home" ? "font-semibold" : "text-white/85"}`}>{label}</button>)}</nav>
        <div className="ml-auto flex items-center gap-3 md:gap-4">
          ...
          <div className="relative"><button type="button" onClick={(event) => { event.stopPropagation(); setProfileOpen((value) => !value); setNoticeOpen(false); }} className="flex items-center gap-2"><img src="https://cdn.whitescreen.dev/spider-man-pfp-spider-man-avatar-built-for-strong-identity-and-recognition-agoz-square_hd-0226_800.webp" alt={profileName} className="h-8 w-8 rounded object-cover"/><span className="hidden text-xs md:block">{profileName}</span><span className="text-[9px]">▼</span></button>{profileOpen && <div onClick={(event) => event.stopPropagation()} className="absolute right-0 top-12 w-52 border-t-2 border-white bg-[#111]/95 py-2 text-sm shadow-2xl"><button onClick={() => navigate("/whos-watching")} className="block w-full px-4 py-2 text-left hover:underline">Manage Profiles</button><button className="block w-full px-4 py-2 text-left hover:underline">Account</button><button className="block w-full px-4 py-2 text-left hover:underline">Help</button><div className="my-1 border-t border-white/15"/><button onClick={() => navigate("/login")} className="block w-full px-4 py-2 text-left hover:underline">Sign Out of STREAM</button></div>}</div>
        </div>
      </div>
    </header>
